...
export const signupSchema = Joi.object({
    ...
    user: Joi.string().required().max(50).message('O campo "usuário", pode ter no máximo {{#limit}} caracteres.'),
    email: Joi.string().email({tlds: {allow: false}}).required().max(100).message('O campo "email", pode ter no máximo {{#limit}} caracteres.'),
    password: Joi.string().required().max(50).min(6).message('O campo "senha", precisa ter no mínimo {{#limit}} caracteres.'),
})
